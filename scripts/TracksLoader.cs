...
public partial class TracksLoader : Node
{
  ...
  private static List<MusicData.Player> ParsePlayers(Godot.FileAccess file, int numPlayers = -1)
  {
    ...
    while (playersRead < numPlayers && !file.EofReached())
    {
      ...
      players.Add(new MusicData.Player { Id = playerId, Name = playerName, Role = role });
  ...
  private static Dictionary<MusicData.PlayerRole, List<MusicData.Note>> ParseBeats(Godot.FileAccess file, List<MusicData.Player> players, int numBeats = -1)
  {
  ...
  private static List<MusicData.Note> ParseNotes(string notesStr, short measure, short beat, short sixteenth)
  {
